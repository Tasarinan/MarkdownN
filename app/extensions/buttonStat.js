...
var buttonStatSettingsBlockHTML = load.loadHtml(
  'buttonStatSettingsBlock.html');
var buttonStatHTML = load.loadHtml(
  'buttonStat.html');
...
buttonStat.onLoadSettings = function () {
  _.each([
    ...
    utils.setInputValue("#input-stat-name" + index, buttonStat.config[
      "name" + index]);
    utils.setInputValue("#input-stat-value" + index, buttonStat.config[
      "value" + index]);
  });
};
...
buttonStat.onSaveSettings = function (newConfig, event) {
  _.each([
    ...
    newConfig["name" + index] = utils.getInputTextValue(
      "#input-stat-name" + index, event);
    newConfig["value" + index] = utils.getInputRegExpValue(
      "#input-stat-value" + index, event);
  });
};
...
buttonStat.onPreviewFinished = function () {
  ...
  var scriptEltList = previewContentsEltClone.getElementsByTagName(
    'script');
  ...
  valueElt.textContent = value1Elt.textContent = (text.match(regex1) || [])
    .length;
