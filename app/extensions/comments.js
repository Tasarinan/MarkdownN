...
var storage = require(path.resolve(__dirname, '../modules/settings/storage'));
...
var commentsPopoverContentHTML = load.loadHtml('commentsPopoverContent.html');
var commentTmpl = [
...
function movePopover(commentElt) {
	...
	popoverElt.querySelector('.arrow').style.left = (marginElt.offsetWidth -
			parseInt(commentElt.style.right) - commentElt.offsetWidth / 2 - left) +
		'px';
	var popoverTopOffset = window.innerHeight - currentContext.hr.getBoundingClientRect()
		.top;
	if (popoverTopOffset < 0) {
		popoverElt.style.top = (parseInt(popoverElt.style.top) + popoverTopOffset) +
			'px';
	}
}
...
var refreshDiscussions = _.debounce(function () {
	...
	yList = [];
	var discussionList = _.sortBy(currentFileDesc.discussionList, function (
		discussion) {
	...
	function refreshOne() {
		...
		if (discussionList.length === 0) {
			...
			if (currentContext && !currentContext.discussionIndex) {
				inputElt.scrollTop += parseInt(newCommentElt.style.top) - inputElt.scrollTop -
					inputElt.offsetHeight * 3 / 4;
				movePopover(newCommentElt);
			}
			sortedCommentEltList = _.sortBy(commentEltMap, function (commentElt) {
				return commentElt.selectionEnd;
			});
			$openDiscussionElt.toggleClass('some', sortedCommentEltList.length !==
				0);
			$openDiscussionElt.toggleClass('replied', someReplies);
			$openDiscussionIconElt.toggleClass('icon-chat', sortedCommentEltList.length !==
				0);
			return;
		}
		...
		var isReplied = !discussion.commentList || !author || _.last(discussion.commentList)
			.author != author;
		...
		if (currentContext && currentContext.getDiscussion() === discussion) {
			inputElt.scrollTop += parseInt(commentElt.style.top) - inputElt.scrollTop -
				inputElt.offsetHeight * 3 / 4;
...
comments.onCommentsChanged = function (fileDesc) {
	...
	if (currentContext !== undefined) {
		...
		if (context.discussionIndex) {
			context.getPopoverElt().querySelector('.discussion-comment-list').innerHTML =
		...
		context.selectionRange = selectionMgr.createRange(discussion.selectionStart,
			discussion.selectionEnd);

		// Highlight selected text
		context.rangyRange = rangy.createRange();
		context.rangyRange.setStart(context.selectionRange.startContainer,
			context.selectionRange.startOffset);
		context.rangyRange.setEnd(context.selectionRange.endContainer, context.selectionRange
			.endOffset);
...
function closeCurrentPopover() {
	currentContext && currentContext.$commentElt.popover('toggle').popover(
		'destroy');
}
...
comments.onDiscussionRemoved = function (fileDesc, discussion) {
	if (currentFileDesc === fileDesc) {
		...
		if (currentContext !== undefined && currentContext.discussionIndex ==
...
comments.onReady = function () {
	cssApplier = rangy.createClassApplier('comment-highlight', {
		normalize: false
	});
	var previousContent = '';
	...
	$(document.body).append($popoverContainer).on('click', function (evt) {
		...
		title: function () {
			...
			var title = editor.getValue().substr(discussion.selectionStart,
				titleLength > 20 ? 20 : titleLength);
			if (titleLength > 20) {
				title += '...';
			}
	...
	$(marginElt).on('show.bs.popover', function (evt) {
		...
		if (!discussion) {
			// Get selected text
			var selectionStart = Math.min(selectionMgr.selectionStart,
				selectionMgr.selectionEnd);
		...
		context.selectionRange = selectionMgr.createRange(discussion.selectionStart,
			discussion.selectionEnd);
		inputElt.scrollTop += parseInt(evt.target.style.top) - inputElt.scrollTop -
			inputElt.offsetHeight * 3 / 4;

	}).on('shown.bs.popover', function (evt) {
		var context = currentContext;
		var popoverElt = context.getPopoverElt();
		context.$authorInputElt = $(popoverElt.querySelector(
			'.input-comment-author')).val(storage['author.name']);
		context.$contentInputElt = $(popoverElt.querySelector(
		...
		$().add(context.$contentInputElt).add(context.$authorInputElt).keydown(
			function (evt) {
				if (evt.which === 13) {
					// Enter key
					evt.preventDefault();
		...
		var $removeButton = $(popoverElt.querySelector(
	...
	$openDiscussionElt = $('.button-open-discussion').click(function (evt) {
		...
		} else if (selectionMgr.selectionStart === selectionMgr.selectionEnd &&
			sortedCommentEltList.length) {
			$commentElt = $(_.first(sortedCommentEltList));
		}
