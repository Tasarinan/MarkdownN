...
var dialogManagePublication = new Extension("dialogManagePublication",
	'Dialog "Manage publication"', false, true);
var dialogManagePublicationLocationHTML = load.loadHtml(
	'dialogManagePublicationLocation.html');
var eventMgr;
...
var refreshDialog = function (fileDescParameter) {
	...
	$showAlreadyPublishedElt.toggleClass("hide", _.size(fileDesc.publishLocations) ===
		0);

	var publishListHtml = _.reduce(fileDesc.publishLocations, function (result,
		publishAttributes) {
		var formattedAttributes = _.omit(publishAttributes, "provider",
			"publishIndex");
		formattedAttributes.password && (formattedAttributes.password =
			"********");
		formattedAttributes = JSON.stringify(formattedAttributes).replace(/{|}|"/g,
			"").replace(/,/g, ", ");
		return result + _.template(dialogManagePublicationLocationHTML, {
			publishAttributes: publishAttributes,
			publishDesc: formattedAttributes,
			publishLocationLink: publishAttributes.provider.getPublishLocationLink &&
				publishAttributes.provider.getPublishLocationLink(publishAttributes)
		});
	}, '');
...
dialogManagePublication.onReady = function () {
	...
	$showAlreadyPublishedElt = $(document.querySelectorAll(
		".show-already-published"));

	$(publishListElt).on('click', '.remove-button', function () {
		var $removeButtonElt = $(this);
		var publishAttributes = fileDesc.publishLocations[$removeButtonElt.data(
			'publishIndex')];
...
module.exports = dialogManagePublication;
