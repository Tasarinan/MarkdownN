...
var dialogManageSharing = new Extension("dialogManageSharing",
	'Button "Share"', false, true);
var dialogManageSharingLocationHTML = load.loadHtml(
	'dialogManageSharingLocation.html');
...
var refreshDocumentSharing = function (fileDescParameter) {
	...
	var editorLinkListHtml = _.reduce(fileDesc.syncLocations, function (result,
		attributes) {
	...
	var viewerLinkListHtml = _.reduce(fileDesc.publishLocations, function (
		result, attributes) {
		var params = sharing.getViewerParams(attributes);
		if (params) {
			var link = config.MAIN_URL + 'viewer#!' + $.param(params);
			result += _.template(dialogManageSharingLocationHTML, {
				link: link
			});
		}
...
module.exports = dialogManageSharing;
