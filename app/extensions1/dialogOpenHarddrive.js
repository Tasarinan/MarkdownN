...
var config = require(path.resolve(__dirname, '../modules/config'));
var _ = require('underscore');
var toMarkdown = require('to-markdown');
var dialogOpenHarddrive = new Extension("dialogOpenHarddrive",
  'Dialog "Open from"');
...
function handleFileImport(evt) {
  ...
  $(".modal-import-harddrive-markdown, .modal-import-harddrive-html").modal(
    "hide");
  _.each(files, function (file) {
    if ($(evt.target).is("#wmd-input *") && file.name.match(
        /.(jpe?g|png|gif)$/i)) {
      return;
    }
    ...
    reader.onload = (function (importedFile) {
      return function (e) {
        ...
        if (content.match(/\uFFFD/)) {
          eventMgr.onError(importedFile.name +
            " is a binary file.");
          return;
        }
        content = contentWrapper ? contentWrapper(content) :
          content;
        if (content === undefined) {
          eventMgr.onError(importedFile.name +
            " is not a valid HTML file.");
          return;
        }
    ...
    var blob = file.slice(0, config.IMPORT_FILE_MAX_CONTENT_SIZE);
...
dialogOpenHarddrive.onReady = function () {
  ...
  $("#input-file-import-harddrive-markdown").change(
...
module.exports = dialogOpenHarddrive;
