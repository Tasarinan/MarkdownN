...
var documentManager = new Extension("documentManager", 'Document Manager',
  false, true);
...
function doSelect() {
  ...
  _.each(documentListElt.querySelectorAll(
    'input[type="checkbox"]:checked'), function (checkboxElt) {
...
function doDeleteConfirmation() {
  ...
  var selectedDocumentListHtml = _.chain(selectedDocumentList).sortBy(
    function (fileDesc) {
  ...
  selectedDocumentListElt.innerHTML = '<ul class="file-list nav">' +
    selectedDocumentListHtml + '</ul>';

  // Ask user confirmation
  $(modalElt.querySelectorAll('.document-list')).addClass('hide');
  $(modalElt.querySelectorAll('.confirm-delete, .selected-document-list'))
    .removeClass('hide');
}
...
var doActiveButtons = _.debounce(function () {
  ...
  $liMoveElt.toggleClass('disabled', _.size(folderList) === 0 || _.size(
    selectedDocumentList) === 0);
  $liDeleteElt.toggleClass('disabled', _.size(selectedFolderList) ===
    0 && _.size(selectedDocumentList) === 0);
}, 50);
...
var refreshManager = _.debounce(function () {
  ...
  var orphanListHtml = _.chain(orphanDocumentList).sortBy(function (
    fileDesc) {
    return fileDesc.title.toLowerCase();
  }).reduce(function (result, fileDesc) {
  ...
  orphanListHtml = orphanListHtml && '<ul class="nav">' +
    orphanListHtml + '</ul>';
  documentListHtml +=
    '<div class="file-list collapse root-folder clearfix">' +
    orphanListHtml + '</div>';
  ...
  _.chain(folderList).sortBy(function (folderDesc) {
    ...
    var fileListHtml = _.chain(folderDesc.fileList).sortBy(
      function (fileDesc) {
        return fileDesc.title.toLowerCase();
      }).reduce(function (result, fileDesc) {
      return result + _.template(documentEltTmpl, {
    ...
    fileListHtml = fileListHtml && '<ul class="nav">' +
      fileListHtml + '</ul>';
...
documentManager.onTitleChanged = function (fileDesc) {
  ...
  $(documentListElt).find('[data-file-index="' + fileDesc.fileIndex +
    '"] .name').html(fileDesc.composeTitle()).removeClass('hide');
...
documentManager.onReady = function () {
  ...
  selectedDocumentListElt = modalElt.querySelector(
    '.list-group.selected-document-list');
  var selectFolderListElt = modalElt.querySelector(
    '.list-group.select-folder-list');
  ...
  $(modalElt.querySelectorAll('.action-create-folder')).click(function () {
    ...
    setTimeout(function () {
      var renameButtonElt = $(modalElt.querySelector(
  ...
  $(modalElt.querySelectorAll('.action-select-all')).click(function () {
    $(documentListElt.querySelectorAll(
      '.folder input[type="checkbox"]')).prop('checked', true).change();
  });
  $(modalElt.querySelectorAll('.action-unselect-all')).click(function () {
    $(documentListElt.querySelectorAll(
      '.folder input[type="checkbox"]')).prop('checked', false).change();
  });

  // Delete selection actions
  var $aDeleteElt = $(modalElt.querySelectorAll('.action-delete-items'))
    .click(function () {
  ...
  $(modalElt.querySelectorAll('.action-delete-items-confirm')).click(
    function () {
      doDelete();

      $(modalElt.querySelectorAll('.document-list')).removeClass(
        'hide');
      $(modalElt.querySelectorAll(
        '.confirm-delete, .selected-document-list')).addClass(
        'hide');
    });

  // Move selection actions
  var $aMoveElt = $(modalElt.querySelectorAll('.action-move-items')).click(
    function () {
      ...
      selectFolderListHtml += _.chain(folderList).sortBy(function (
        folderDesc) {
        return folderDesc.name.toLowerCase();
      ...
      _.each(selectFolderListElt.querySelectorAll('.folder'),
        function (folderElt) {
          folderElt = $(folderElt);
          folderElt.click(function () {
            var folderDesc = folderList[folderElt.data(
              'folderIndex')];
            _.each(selectedDocumentList, function (fileDesc) {
              fileDesc.folder && fileDesc.folder.removeFile(
                fileDesc);
            ...
            $(modalElt.querySelectorAll('.document-list')).removeClass(
              'hide');
            $(modalElt.querySelectorAll(
              '.choose-folder, .select-folder-list')).addClass(
              'hide');
          });
        });
      ...
      $(modalElt.querySelectorAll('.document-list')).addClass('hide');
      $(modalElt.querySelectorAll(
        '.choose-folder, .select-folder-list')).removeClass('hide');
    });
  ...
  $(modalElt.querySelectorAll('.action-cancel')).click(function () {
    $(modalElt.querySelectorAll('.document-list')).removeClass(
      'hide');
    $(modalElt.querySelectorAll(
      '.confirm-delete, .choose-folder, .selected-document-list, .select-folder-list'
    )).addClass('hide');
  });
  ...
  $(documentListElt).on('click', '.button-delete', function (evt) {
    ...
    $parentElt.find('.input-rename').removeClass('hide').val(name)[
      0].select();
    ...
    $(this).parents('.file-list').prev().find('.checkbox input').prop(
      'checked', false);
    doActiveButtons();
  }).on('click', '.folder .checkbox', function (evt) {
    evt.stopPropagation();
  }).on('change', '.folder .checkbox input', function () {
    $(this).parents('.folder').next().find('.checkbox input').prop(
