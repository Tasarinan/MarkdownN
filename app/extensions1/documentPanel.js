...
var refreshPanel = _.debounce(function () {
  ...
  var documentListHtml = _.chain(orphanDocumentList).sortBy(function (
    fileDesc) {
  ...
  documentListHtml = documentListHtml && '<ul class="nav">' +
    documentListHtml + '</ul>';
  ...
  var documentListFilteredHtml = _.chain(fileSystem).sortBy(function (
    fileDesc) {
    return fileDesc.title.toLowerCase();
  }).reduce(function (result, fileDesc) {
    return result + '<li>' + _.template(documentEltTmpl, {
  ...
  documentListFilteredHtml = '<ul class="nav">' +
    documentListFilteredHtml + '</ul>';
...
function filterFiles(filterValue) {
  ...
  _.each(documentListFilteredElt.querySelectorAll('.file'), function (
    fileElt) {
    var $fileElt = $(fileElt);
...
documentPanel.onReady = function () {
  ...
  documentListFilteredElt = panelElt.querySelector(
  ...
  $(panelElt).on('show.layout.toggle', function () {
    ...
    if (folderDesc !== undefined) {
      $(panelElt.querySelector('.file-list.' + folderDesc.folderIndex
        .replace('.', ''))).collapse('show');
    }
  }).on('shown.layout.toggle', function () {
    // Scroll to the active file
    var activeElt = documentListElt.querySelector('.file.active');
    activeElt && (panelContentElt.scrollTop += activeElt.getBoundingClientRect()
      .top - 240);
  ...
  var $filterInputElt = $(panelElt.querySelector(
