...
var findReplace = new Extension("findReplace", 'Find and Replace', true,
	true);
var findReplaceHTML = load.loadHtml('findReplace.html');
var findReplaceSettingsBlockHTML = load.loadHtml(
	'findReplaceSettingsBlock.html');
...
findReplace.onSaveSettings = function (newConfig, event) {
	newConfig.findReplaceShortcut = utils.getInputTextValue(
		"#input-find-replace-shortcut", event);
};
...
function highlight(force) {
	...
	if (!force && text == previousText && caseSensitive ==
		previousCaseSensitive && useRegexp == previousUseRegexp) {
		return;
	}
	previousText = text;
	...
	if (textLength) {
		try {
			...
			text = useRegexp ? text : text.replace(
				/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
...
function show() {
	...
	shown = true;
	$findReplaceElt.show();
	$searchForInputElt.focus()[0].setSelectionRange(0, $searchForInputElt.val()
		.length);
...
function find() {
	resetSelect();
	var position = Math.min(editor.selectionMgr.selectionStart, editor.selectionMgr
		.selectionEnd);
...
findReplace.onReady = function () {
	...
	$caseSensitiveElt = $findReplaceElt.find('.checkbox-case-sensitive').change(
		_.bind(highlight, null, false));
	$regexpElt = $findReplaceElt.find('.checkbox-regexp').change(_.bind(
		highlight, null, false));
	$findReplaceElt.find('.search-button').click(find);
	$searchForInputElt = $('#input-find-replace-search-for').keyup(_.bind(
		highlight, null, false));
	...
	// Key bindings
	$().add($searchForInputElt).add($replaceWithInputElt).keydown(function (
		evt) {
		if (evt.which === 13) {
			// Enter key
			evt.preventDefault();
