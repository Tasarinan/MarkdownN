  ...
  var notificationsSettingsBlockHTML = load.loadHtml(
    'notificationsSettingsBlock.html');
  ...
  notifications.onLoadSettings = function () {
    utils.setInputValue("#input-notifications-timeout", notifications.config
      .timeout);
  };

  notifications.onSaveSettings = function (newConfig, event) {
    newConfig.timeout = utils.getInputIntValue(
      "#input-notifications-timeout", event, 1, 60000);
  };
  ...
  function showMessage(message, iconClass, options) {
    ...
    jGrowl("<i class='icon-white " + iconClass + "'></i> " + _.escape(
      message).replace(/\n/g, '<br/>'), options);
  }
  ...
  notifications.onOfflineChanged = function (isOfflineParam) {
    ...
    if (isOffline === true) {
      showMessage("You are offline.",
        "icon-attention-circled msg-offline");
    } else {
  ...
  notifications.onSyncExportSuccess = function (fileDesc, syncAttributes) {
    showMessage('"' + fileDesc.title + '" will now be synchronized on ' +
      syncAttributes.provider.providerName + '.');
  };

  notifications.onSyncRemoved = function (fileDesc, syncAttributes) {
    showMessage(syncAttributes.provider.providerName +
  ...
  notifications.onNewPublishSuccess = function (fileDesc, publishAttributes) {
    showMessage('"' + fileDesc.title + '" is now published on ' +
      publishAttributes.provider.providerName + '.');
  };

  notifications.onPublishRemoved = function (fileDesc, publishAttributes) {
    showMessage(publishAttributes.provider.providerName +
      " publish location has been removed.");
  };
