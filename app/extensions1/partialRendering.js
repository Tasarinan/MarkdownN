...
var markdownExtra = require('./markdownExtra');

var load = require(path.resolve(__dirname, '../modules/load'));
var partialRenderingSettingsBlockHTML = load.loadHtml(
	'partialRenderingSettingsBlock.html');

var partialRendering = new Extension("partialRendering", "Partial Rendering",
	true);
...
function updateSectionList() {
	...
	_.some(sectionList, function (section, index) {
		if (index >= newSectionList.length || section.text != newSectionList[
				index].text) {
			leftIndex = index;
			return true;
		}
	...
	_.some(sectionList.slice().reverse(), function (section, index) {
		if (index >= newSectionList.length || section.text != newSectionList[
				newSectionList.length - index - 1].text) {
			rightIndex = -index;
			return true;
		}
	});
	...
	modifiedSections = newSectionList.slice(leftIndex, newSectionList.length +
		rightIndex);
	var rightSections = sectionList.slice(sectionList.length + rightIndex,
		sectionList.length);
	insertBeforeSection = _.first(rightSections);
	sectionsToRemove = sectionList.slice(leftIndex, sectionList.length +
		rightIndex);
...
function refreshSections() {
	...
	_.each(sectionsToRemove, function (section) {
		var sectionElt = document.getElementById("wmd-preview-section-" +
			section.id);
	...
	if (insertBeforeSection !== undefined) {
		insertBeforeElt = document.getElementById("wmd-preview-section-" +
			insertBeforeSection.id);
	}
	...
	if (hasFootnotes === true) {
		...
		_.each(previewContentsElt.querySelectorAll('a.footnote'), function (elt,
			index) {
...
module.exports = partialRendering;
