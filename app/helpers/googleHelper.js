...
define([
	...
	var scopeMap = {
		...
		gdrive: [
			...
			settings.gdriveFullAccess === true ? 'https://www.googleapis.com/auth/drive' :
	...
	function authenticate(task, permission, accountId) {
		...
		task.onRun(function () {
			...
			function localAuthenticate() {
				if (authuser > 5) {
					return task.error(new Error('Unable to authenticate user ' + authorizationMgr.getUserId() +
	...
	googleHelper.uploadBlogger = function (blogUrl, blogId, postId, labelList, isDraft, publishDate, title, content,
		callback) {
