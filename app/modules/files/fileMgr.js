...
var core = require(path.resolve(__dirname, '../core'));
...
fileMgr.selectFile = function (fileDesc) {
  ...
  if (fileDesc === undefined) {
    ...
    if (fileSystemSize === 0) {
      ...
      fileDesc = fileMgr.createFile(config.WELCOME_DOCUMENT_TITLE, "welcomeContent");
    } else {
  ...
  if (fileMgr.currentFile !== fileDesc) {
    ...
    messenger.publish.extension("onFileSelected", fileDesc);

    // Hide the viewer pencil button
    $(".action-edit-document").toggleClass("hide", fileDesc.fileIndex != config.TEMPORARY_FILE_INDEX);
  }
...
fileMgr.createFile = function (title, content, discussionListJSON, syncLocations, isTemporary) {
  ...
  if (!isTemporary) {
    ...
    messenger.publish.extension("onFileCreated", fileDesc);
  }
  return fileDesc;
};
...
fileMgr.deleteFile = function (fileDesc) {
  ...
  if (fileDesc.folder) {
    ...
    messenger.publish.extension("onFoldersChanged");
  }
  ...
  messenger.publish.extension("onFileDeleted", fileDesc);
};
...
messenger.subscribe.extension('onReady', function () {
  ...
  $(".action-welcome-file").click(function () {
    var fileDesc = fileMgr.createFile(config.WELCOME_DOCUMENT_TITLE, "welcomeContent");
    fileMgr.selectFile(fileDesc);
  });
});

//eventMgr.onFileMgrCreated(fileMgr);
messenger.publish.extension("onFileMgrCreated", fileMgr);
