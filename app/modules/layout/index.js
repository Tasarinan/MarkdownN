...
var navbar, menuPanel, documentPanel, editor, previewPanel, previewContainer,
	navbarToggler, previewToggler, previewResizer, previewButtons;
...
DomObject.prototype.applyCss = function () {
	...
	if (this.x !== undefined || this.y !== undefined) {
		this.x = this.x || 0;
		this.y = this.y || 0;
		this.elt.style['-webkit-transform'] = 'translate(' + this.x + 'px, ' +
			this.y + 'px)';
		this.elt.style['-ms-transform'] = 'translate(' + this.x + 'px, ' + this.y +
			'px)';
		this.elt.style.transform = 'translate(' + this.x + 'px, ' + this.y +
			'px)';
	}
...
DomObject.prototype.createToggler = function (backdrop) {
	...
	this.toggle = function (show) {
		...
		if (this.isOpen) {
			...
			if (backdrop) {
				$backdropElt = $(utils.createBackdrop(wrapperL1.elt)).on(
					'click.backdrop', _.bind(function () {
						this.toggle(false);
					}, this));
			...
			transitionEndCallbacks.push(_.bind(function () {
				if (--pushedEvents === 0) {
					if (!this.isOpen) {
						this.isShown = false;
						this.$elt.removeClass('panel-open bring-to-front').trigger(
...
var maxWidthMap = [{
	screenWidth: 0,
	maxWidth: 600 * settings.maxWidthRatio
}, {
	screenWidth: 1000,
	maxWidth: 700 * settings.maxWidthRatio
}, {
	screenWidth: 1200,
	maxWidth: 800 * settings.maxWidthRatio
}, {
	screenWidth: 1400,
	maxWidth: 900 * settings.maxWidthRatio
}];
...
function onResize() {
	...
	if (editorPadding < config.EDITOR_DEFAULT_PADDING) {
		editorPadding = config.EDITOR_DEFAULT_PADDING;
	}
	...
	if (previewPadding < config.EDITOR_DEFAULT_PADDING) {
		previewPadding = config.EDITOR_DEFAULT_PADDING;
	}
	...
	var maxWidth = navbarMarginWidth + workingIndicatorWidth + titleMinWidth +
		buttonsDropdownWidth;
	if (window.viewerMode) {
		maxWidth = navbarMarginWidth + workingIndicatorWidth + titleMinWidth +
			viewerButtonGroupWidth;
	}
	var titleWidth = windowSize.width - maxWidth + titleMinWidth;
	navbarBtnGroups.forEach(function (group, index) {
		maxWidth += group.width;
		index === navbarBtnGroups.length - 1 && (maxWidth -=
			buttonsDropdownWidth);
...
function fixViewportScrolling() {
	...
	wrapperL1.width = windowSize.width + menuPanelWidth + (documentPanel.isShown ?
		documentPanelWidth : 0);
...
function resizeAll() {
	...
	while (true) {
		...
		wrapperL1.x = menuPanel.isOpen ? 0 : documentPanel.isOpen ? -(
			menuPanelWidth + documentPanelWidth) : -menuPanelWidth;
			...
			if (!previewPanel.isOpen) {
				...
				if (previewPanel.y < editorMinSize.height) {
					...
					if (previewPanelHeight < previewMinSize.height) {
						...
						previewPanel.$elt.trigger('hide.layout.toggle').trigger(
							'hidden.layout.toggle');
						continue;
					}
...
module.init = function () {
	...
	navbarDropdownElt = navbar.elt.querySelector(
	...
	_.each(navbar.elt.querySelectorAll('.right-buttons'), function (
		btnGroupElt) {
	...
	documentPanel.$elt.find('.toggle-button').click(_.bind(documentPanel.toggle,
		documentPanel));

	// Hide panels when clicking on a non collapse element
	documentPanel.$elt.on('click', 'a[data-toggle!=collapse]', _.bind(
		documentPanel.toggle, documentPanel, false));
	...
	if (!window.viewerMode) {
		...
		menuPanel.$elt.find('.toggle-button').click(_.bind(menuPanel.toggle,
			menuPanel));

		// Hide panels when clicking on a non collapse element
		menuPanel.$elt.on('click', 'a[data-toggle!=collapse]', _.bind(menuPanel.toggle,
			menuPanel, false));
...
messenger.subscribe.extension('onReady', function () {
	...
	openPreviewButtons();
	closePreviewButtons();
	previewButtons.$elt.hover(openPreviewButtons, closePreviewButtons).on(
		'show.bs.dropdown',
		function () {
			dropdownOpen = true;
	...
	_.each(previewButtons.elt.querySelectorAll('.btn-group'), function (
		btnGroupElt) {
		...
		$btnGroupElt.on('shown.bs.dropdown', function () {
			...
			$btnGroupElt.find('.dropdown-menu').css({
				right: -previewButtons.elt.offsetWidth + $btnGroupElt.width() +
					$btnGroupElt.position().left
			});
			var maxHeight = -previewButtons.y - previewButtonsDropdownMargin;
			// If it's a dropup
			if (windowSize.height / 2 > -previewButtons.y) {
				maxHeight = windowSize.height + previewButtons.y - previewButtons.elt
					.offsetHeight - previewButtonsDropdownMargin;
			}
