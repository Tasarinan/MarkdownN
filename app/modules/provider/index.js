var
  ...
  editor = require(path.resolve(__dirname, '../editor')),
  fileMgr = require(path.resolve(__dirname, '../files/fileMgr')),
...
Provider.prototype.syncMerge = function (fileDesc, syncAttributes, remoteContent, remoteTitle, remoteDiscussionList,
  ...
  if (
    ...
    messenger.publish.extension("onMessage", 'Conflict detected on "' + localTitle +
      '". A backup has been created locally.');
  } else {
    if (contentConflict) {
      // Patch content
  ...
  if (titleChanged) {
    ...
    messenger.publish.extension("onTitleChanged", fileDesc);
    //eventMgr.onMessage('"' + localTitle + '" has been renamed to "' + newTitle + '" on ' + this.providerName + '.');
    messenger.publish.extension("onMessage", '"' + localTitle + '" has been renamed to "' + newTitle + '" on ' + this
      .providerName +
      '.');
  ...
  if (contentChanged || discussionListChanged) {
    editor.watcher.noWatch(_.bind(function () {
      if (contentChanged) {
        ...
        messenger.publish.extension("onContentChanged", fileDesc, newContent);
      }
      if (discussionListChanged) {
        ...
        _.each(diff, function (discussionDiff, discussionIndex) {
          if (!_.isArray(discussionDiff)) {
            ...
            messenger.publish.extension("onDiscussionCreated", fileDesc, newDiscussionList[discussionIndex]);
          } else {
            //eventMgr.onDiscussionRemoved(fileDesc, localDiscussionList[discussionIndex]);
            messenger.publish.extension("onDiscussionRemoved", fileDesc, localDiscussionList[
              discussionIndex]);
      ...
      messenger.publish.extension("onMessage", '"' + remoteTitle + '" has been updated from ' + this.providerName +
        '.');
      if (conflictList.length) {
        //    eventMgr.onMessage('"' + remoteTitle + '" has conflicts that you have to review.');
        messenger.publish.extension("onMessage", '"' + remoteTitle + '" has conflicts that you have to review.');
