...
var Provider = require(path.resolve(__dirname, '../provider'));
var messenger = require(path.resolve(__dirname, '../messenger'));
var fileSystem = require(path.resolve(__dirname, '../files/fileSystem'));
var fileMgr = require(path.resolve(__dirname, '../files/fileMgr'));
...
var providerMap = require('require-all')({
  dirname: path.resolve(__dirname, '../../providers'),
  excludeDirs: /^\.(git|svn)$/,
  recursive: false,
  ...
  _.each(fileSystem, function (fileDesc) {
    utils.retrieveIndexArray(fileDesc.fileIndex + ".sync").forEach(function (syncIndex) {
      try {
        ...
        //eventMgr.onError(e);
        messenger.publish.extension("onError", e);
...
function autosyncAll(callback) {
  ...
  function fileAutosync() {
    ...
    function providerAutosync() {
      ...
      provider.autosyncFile(fileDesc.title, fileDesc.content, fileDesc.discussionListJSON, provider.autosyncConfig,
        function (error, syncAttributes) {
          if (error) {
            return callback(error);
          }
          fileDesc.addSyncLocation(syncAttributes);
          //eventMgr.onSyncExportSuccess(fileDesc, syncAttributes);
          messenger.publish.extension("onSyncExportSuccess", fileDesc, syncAttributes);
          providerAutosync();
        });
    }
...
messenger.subscribe.extension("onOfflineChanged", function (isOfflineParam) {
...
synchronizer.sync = function () {
  ...
  messenger.publish.extension("onSyncRunning", true);
  ...
  function isError(error) {
    if (error !== undefined) {
      ...
      messenger.publish.extension("onSyncRunning", false);
  ...
  autosyncAll(function (error) {
    ...
    syncDown(function (error) {
      ...
      syncUp(function (error) {
        ...
        messenger.publish.extension("onSyncSuccess", false);
      });
    });
  });
  return true;
...
messenger.subscribe.extension("onFileCreated", function (fileDesc) {
...
messenger.subscribe.extension("onReady", function () {
  ...
  _.each(providerMap, function (provider) {
    ...
    $(".action-sync-export-" + provider.providerId).click(function (event) {
      ...
      provider.exportFile(event, fileDesc.title, fileDesc.content, fileDesc.discussionListJSON, fileDesc.frontMatter,
        function (error, syncAttributes) {
          if (error) {
            return;
          }
...
messenger.publish.extension("onSynchronizerCreated", synchronizer);
