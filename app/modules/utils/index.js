...
utils.msie = (function () {
	 ...
	 */
	var msie = parseInt((/msie (\d+)/.exec(navigator.userAgent.toLowerCase()) || [])[
		1], 10);
	if (isNaN(msie)) {
		msie = parseInt((/trident\/.*; rv:(\d+)/.exec(navigator.userAgent.toLowerCase()) || [])[
			1], 10);
	}
	return msie;
})();
...
utils.getInputTextValue = function (element, event, validationRegex) {
	...
	if ((value.length === 0) || (validationRegex !== undefined && !value.match(
			validationRegex))) {
		inputError(element, event);
		return undefined;
	}
...
function getInputNumValue(isFloat, element, event, min, max) {
	...
	if (isNaN(value) || (min !== undefined && value < min) || (max !==
			undefined && value > max)) {
		inputError(element, event);
		return undefined;
	}
...
utils.setInputRadio = function (name, value) {
	$("input:radio[name=" + name + "][value=" + value + "]").prop("checked",
		true).change();
};
...
utils.resetModalInputs = function () {
	$(
		".modal input[type=text]:not([disabled]), .modal input[type=password], .modal textarea"
	).val("");
...
utils.saveAs = function (content, filename) {
	if (saveAs !== undefined && !/constructor/i.test(window.HTMLElement) /* safari does not support saveAs */ ) {
		...
		if (_.isString(content)) {
			var uriContent = "data:application/octet-stream;base64," + utils.encodeBase64(
				content);
			...
			reader.onload = function (event) {
				utils.redirectConfirm('You are opening a PDF document.', function () {
					var uriContent = 'data:application/pdf;' + event.target.result.substring(
...
utils.storeAttributes = function (attributes) {
	...
	var storedAttributes = _.omit(attributes, "syncIndex", "publishIndex",
		"provider");
...
utils.formatDate = function (date) {
	...
	parts.forEach(function (value) {
		...
		text += fn ? fn(date, DATETIME_FORMATS) : value.replace(/(^'|'$)/g, '')
			.replace(/''/g, "'");
	});
...
utils.encodeBase64 = function (str) {
	...
	case 2:
		...
		x.push(alpha.charAt(b10 >> 18) + alpha.charAt((b10 >> 12) & 0x3F) + alpha
			.charAt((b10 >> 6) & 0x3f) + padchar);
		break;
	}
	return x.join('');
};
