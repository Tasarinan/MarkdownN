define([
  ...
  bloggerProvider.publish = function (publishAttributes, frontMatter, title,
    content, callback) {
    var labelList = publishAttributes.labelList || [];
    if (frontMatter) {
      frontMatter.tags !== undefined && (labelList = frontMatter.tags);
    }
    _.isString(labelList) && (labelList = _.compact(labelList.split(
      /[\s,]/)));
    ...
    googleHelper.uploadBlogger(publishAttributes.blogUrl,
      publishAttributes.blogId, publishAttributes.postId, labelList,
      isDraft, publishDate, title, content,
  ...
  bloggerProvider.newPublishAttributes = function (event) {
    ...
    publishAttributes.postId = utils.getInputTextValue(
      "#input-publish-postid");
