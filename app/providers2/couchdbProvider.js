define([
...
], function ($, _, crel, alertify, constants, utils, storage, logger,
	Provider, settings, eventMgr, fileMgr, fileSystem, editor, couchdbHelper) {
	...
	function importFilesFromIds(ids, cb) {
		...
		_.each(ids, function (id) {
			...
			if (fileDesc !== undefined) {
				return eventMgr.onError('"' + fileDesc.title +
					'" is already in your local documents.');
			}
			importIds.push(id);
		});
		if (ids.length === 0) {
		...
		couchdbHelper.downloadContent(importIds.map(function (id) {
			...
			_.each(result, function (file) {
				...
				var syncLocations;
				var syncAttributes = createSyncAttributes(file._id, file._rev,
					parsedContent.content, file.title, parsedContent.discussionListJSON
				);
				syncLocations = {};
				syncLocations[syncAttributes.syncIndex] = syncAttributes;
				fileDesc = fileMgr.createFile(file.title, parsedContent.content,
					parsedContent.discussionListJSON, syncLocations);
	...
	couchdbProvider.exportFile = function (event, title, content,
		discussionListJSON, frontMatter, callback) {
		var data = couchdbProvider.serializeContent(content, discussionListJSON);
		var tags = getTags(frontMatter, title);
		couchdbHelper.uploadDocument(undefined, title, data, tags, undefined,
			function (error, result) {
				if (error) {
					return callback(error);
				}
				var syncAttributes = createSyncAttributes(result.id, result.rev,
					content, title, discussionListJSON);
				callback(undefined, syncAttributes);
			});
	};

	couchdbProvider.syncUp = function (content, contentCRC, title, titleCRC,
		discussionList, discussionListCRC, frontMatter, syncAttributes, callback) {
		if (
		...
		var tags = getTags(frontMatter, title);
		couchdbHelper.uploadDocument(syncAttributes.id, title, data, tags,
			syncAttributes.rev,
			function (error, result) {
				if (error) {
	...
	couchdbProvider.syncDown = function (callback) {
		var lastChangeId = parseInt(storage[PROVIDER_COUCHDB + ".lastChangeId"],
			10);
		var syncLocations = {};
		_.each(fileSystem, function (fileDesc) {
			_.each(fileDesc.syncLocations, function (syncAttributes) {
				syncAttributes.provider === couchdbProvider && (syncLocations[
					syncAttributes.id] = syncAttributes);
			});
		});
		couchdbHelper.checkChanges(lastChangeId, syncLocations, function (error,
			changes, newChangeId) {
			if (error) {
				return callback(error);
			}
			...
			couchdbHelper.downloadContent(interestingChanges, function (error,
				changes) {
				if (error) {
					return callback(error);
				}
				...
				function mergeChange() {
					...
					if (change.deleted === true) {
						eventMgr.onError('"' + fileDesc.title +
							'" has been removed from CouchDB.');
					...
					var remoteCRC = couchdbProvider.syncMerge(fileDesc, syncAttributes,
						remoteContent, remoteTitle, remoteDiscussionList,
						remoteDiscussionListJSON);
	...
	eventMgr.addListener("onReady", function () {
		...
		var $selectedDocumentListElt = $(modalElt.querySelector(
		...
		function doSelect() {
			...
			$(modalElt.querySelectorAll('.action-delete-items')).parent().toggleClass(
		...
		function setMode(mode) {
			$(modalElt.querySelectorAll('.list-mode')).toggleClass('hide', mode !=
				'list');
			$(modalElt.querySelectorAll('.delete-mode')).toggleClass('hide', mode !=
				'delete');
			$(modalElt.querySelectorAll('.byid-mode')).toggleClass('hide', mode !=
				'byid');
		}
		...
		var updateDocumentList = _.debounce(function () {
			...
			couchdbHelper.listDocuments($selectTagElt.val(), lastDocument &&
		...
		$(modalElt)
			...
			.on('click', '.action-remove-tag', function () {
				...
				tag && alertify.confirm('You are removing <b>' + $selectTagElt.val() +
					'</b> from your list of filters.',
					function (e) {
						if (e) {
